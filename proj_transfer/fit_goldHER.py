...
class RevPcet2(ElecMech.ElecMech):
...
if __name__ == "__main__":
    ...
    # Some example initial parameters for testing the model
    sig = 0.6  # overall quantity of disorder (how spread parallel cycles are from each other)
    cyc_count = 7  # number of parallel cycles that model a discrete amount of disorder
    ...
    deltas *= 0.5*sig*2**0.5  # width of the cycle energy levels from each other

    fig, ax = plt.subplots()
    data = pd.read_csv('data_csv/Jackson_HERfull_data.csv', names=['logJ','VvsNHE'])

    # I found it easier to fit the data by splitting it by pH, but ultimately the final model should not
    #data = data[data["pH"] != 4]
    #data = data[data["pH"] > 4]
