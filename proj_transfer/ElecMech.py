...
class ElecMech(object):
    ...
    def lograte(s, V):
        # Caclulate the rate from the applied Voltage, and take log10 of the absolute value of the rate
        J = s.rate(V)
        return np.log10(np.abs(J))

    @staticmethod
        # Calculate the rate for a 2-step reversible cycle mechanism
    ...
    @pH.setter
    def pH(self, value):
        # Automatically update the value of self.H when pH is updated
        self.H = 10.**(-value)
        self._pH = value

    def rate(s, V):
        ## Calculates rate from V. To be defined in child class
        pass

    def setConsts(s, p):
        # Sets the rate constants to p. To be defined in child class. 
        # This allows for reuse of code in function calls to other libraries
        pass

    def setConc(s, p):
        ## Sets concentrations (pH, O2) from p. To be defined in child class
        pass
